...
export default function DoctorPatientsPage() {
  ...
  const _selectedPatient = patients.find((p) => p.id === selectedPatientId)

  // Patient detail view
  if (selectedPatientId && patientRecords) {
    return (
              ...
              {patientRecords.patient.patientProfile && (
                <div className="flex gap-4 mt-2 text-xs text-muted-foreground">
                  ...
                  {patientRecords.patient.patientProfile.weight && (
                    <span>
                      Weight: {patientRecords.patient.patientProfile.weight} kg
                    </span>
                  )}
                  {patientRecords.patient.patientProfile.height && (
                    <span>
                      Height: {patientRecords.patient.patientProfile.height} cm
                    </span>
                  )}
                </div>
              )}
  ...
  // Loading detail
  if (selectedPatientId && recordsPending) {
    return (
      <div className="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-4">
        <div className="flex items-center gap-3">
          <Button
