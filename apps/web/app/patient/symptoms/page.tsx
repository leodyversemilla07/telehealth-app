...
const SEVERITY_CONFIG: Record<string, { color: string; icon: typeof Clock; bg: string }> = {
  low: { color: "text-emerald-600", icon: CheckCircle, bg: "bg-emerald-50 border-emerald-200" },
  moderate: { color: "text-amber-600", icon: Clock, bg: "bg-amber-50 border-amber-200" },
  high: { color: "text-orange-600", icon: AlertTriangle, bg: "bg-orange-50 border-orange-200" },
  urgent: { color: "text-red-600", icon: AlertCircle, bg: "bg-red-50 border-red-200" },
}
    ...
    <div className="flex h-full flex-1 flex-col gap-6 overflow-x-auto rounded-xl p-4">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Describe your symptoms and get AI-powered health insights and doctor recommendations.
        </p>
      </div>
      ...
      <Card className="border border-border/40">
        <CardHeader>
          ...
          <CardDescription className="text-xs">
            Be as detailed as possible for better analysis. Include duration, severity, and any relevant medical history.
          </CardDescription>
        </CardHeader>
      ...
      {result && (
        <div className="space-y-6">
          {/* Severity Badge */}
          <div className={`flex items-center gap-3 p-4 rounded-xl border ${severityConfig?.bg || "bg-muted"}`}>
            {severityConfig && <severityConfig.icon className={`h-6 w-6 ${severityConfig.color}`} />}
            <div>
              <p className="text-sm font-semibold">Severity Assessment</p>
              <p className={`text-lg font-bold capitalize ${severityConfig?.color || ""}`}>
                {result.severity}
              </p>
            </div>
          </div>
              ...
              <CardContent>
                <div className="space-y-2">
                  {result.doctors.map((doctor) => (
                    <div key={doctor.id} className="flex items-center justify-between p-3 bg-muted/30 rounded-lg">
                      <div className="flex items-center gap-3">
                        <div className="h-8 w-8 rounded-full bg-primary/10 flex items-center justify-center text-primary font-bold text-xs">
                          {doctor.user.name?.[0] || "?"}
                        </div>
                        <div>
                          <p className="text-sm font-medium">{doctor.user.name}</p>
                          <p className="text-[10px] text-muted-foreground">{doctor.specialty}</p>
                        </div>
                      </div>
                      <Button size="sm" variant="outline" render={<Link href="/patient/appointments/book" />}>
                        Book
                      </Button>
                    </div>
                  ))}
